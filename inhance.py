...
# 读入图片
im = cv2.imread('2.tif', 0)
im_copy = cv2.imread('2.tif', 0)
# 如果图片为空，返回错误信息，并终止程序
if im is None:
    print("图片打开失败！")
    exit()
...
img4 = Contrast_and_Brightness(1.3, 25, img3)
...
plt.title('before', fontproperties=font)

# 显示处理过的图像
plt.subplot(122)  # 子图2
# 显示处理后的图，设置标题和字体
plt.imshow(img4, plt.cm.gray)
plt.title('after', fontproperties=font)
plt.show()
# 销毁所有窗口
